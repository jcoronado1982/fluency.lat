...
└── api_main/        ← composition root:
    ...
    ├── src/modules/            route registration PER module (flashcards.rs, payments.rs, shell.rs)
...
- **SurrealDB 3.2.3** via WS (`SURREAL_URL`; in prod `10.128.0.5:8080` via GCP private VPC — previously `10.0.1.138:8080` in Oracle, migrated Aug 4, 2026, see `tools/oracle-legacy/README.md`).
  ...
  ⚠️ The pool watchdog probes with `SurrealConnection::is_usable()` (`INFO FOR DB`), **never** `db.health()`. `health()` is an unauthenticated ping: it answers OK on a connection whose root session is gone, so it reported healthy while handlers failed with "Anonymous access not allowed" and the backend silently degraded to disk-only reads — 11 times in an hour under load, with zero reconnections. A health-check must exercise what the handlers exercise, credentials included. Note SurrealDB returns per-statement errors *inside* the response, not as `Err` from `query()`: the probe checks `take_errors()`, without which it would be just as blind. The watchdog probes the primary directly (not via `db()`, which round-robins) and every secondary. Regression test: `connection.rs` › `health_misses_a_lost_session_but_is_usable_catches_it` (skips without a local DB). ⚠️ This fixes a *persistent* session loss, which would previously never have been detected — it does **not** fix the intermittent `Specify a namespace to use` / `Session not found` warnings still seen under load (~0.3% of requests, 25 in 14 min, zero reconnections): those come from the SDK reopening the socket without replaying `signin`/`use_ns`, and need a retry in the adapters. Detail and measurements in `scripts/troubleshooting_library.skill.md` §21.
