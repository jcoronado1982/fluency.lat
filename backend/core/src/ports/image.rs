...
pub trait ImageGenerator: Send + Sync {
    ...
    ///
    /// ⚠️ El preámbulo NO debe fijar la hora del día. Antes decía "indoor/outdoor daylight" y, al
    /// ir ANTES de la descripción, forzaba luz diurna aunque la escena fuera nocturna: la frase
    /// "Can you wake me up at 7 AM?" se dice de noche, pero salía de día. La iluminación la manda
    /// la descripción visual; aquí solo se piden calidad y exposición. Ver
    /// `docs/prompt-temporal-precision.md`.
    fn finalize_prompt(&self, visual_description: &str, scene_complement: Option<&str>) -> String {
        let mut prompt = format!(
            "A natural, unedited candid snapshot, shot on 35mm lens, f/2.8 aperture, soft diffused ambient light matching the time of day described in the scene. Balanced natural exposure with rich midtones — NOT overexposed, NOT washed out, NO blown-out white window highlights, NO harsh camera flash flare. Neutral skin tones with healthy balanced complexions — NOT reddish, NOT flushed, NOT oversaturated. Muted realistic color profile, gentle even shadows across faces and walls — NOT high-contrast, NOT studio bright, NOT digitally retouched. Authentic skin textures, realistic pores, subtle natural highlights. Avoid CGI-clean surfaces, avoid artificial white glare, avoid airbrushed skin: {}. An unposed, everyday life scene in a realistic setting. No text, no words, no captions, no signage, no watermarks.",
            visual_description
        );
